...
from isodesign.base.score import Score
...
class Process:
    ...
    def __init__(self):
        ...
        self.summary_dataframe_styler = None
    ...
    def get_path_input_folder(self, directory_path):
        ...
        self.logger.debug(f"Input folder directory = {self.input_folder_path}")
        ...
        self.logger.debug(f"Prefixes from netw files in the folder = {self.netw_files_prefixes}")
    ...
    def load_mtf_files(self, prefix):
        ...
        self.logger.debug(f"Imported files dictionary = {self.imported_files_dict}")
    ...
    def input_network_analysis(self):
        ...
        self.logger.debug(f"self.netan dictionary keys : {self.netan.keys()}")
    ...
    def copy_files(self):
        ...
        """
        self.logger.info("Copy of the imported files to folder containing linp files...")

        for path in self.imported_files_dict.values():
            self.logger.debug(f"path {path[0]}")
            shutil.copy(path[0], self.res_folder_path)
        self.logger.info("Files have been copied \n")
    ...
    def generate_combinations(self, isotopomers_group: dict):
        ...
        self.logger.debug(f"Isotopomers : {self.labelinput.names} {self.labelinput.labelling_patterns}\n")
        self.logger.debug(f"Isotopomers combinations : {self.labelinput.isotopomer_combination}")
    ...
    def generate_linp_files(self):
        ...
        self.res_folder_path = Path(self.input_folder_path/"tmp")
        self.res_folder_path.mkdir(parents=True, exist_ok=True)

        # self.logger.debug(f"path isodesign folder {self.res_folder_path}")
        self.logger.info("Creation of the linp files...")
        ...
        with open(os.path.join(str(self.res_folder_path), 'files_combinations.txt'), 'w', encoding="utf-8") as f:
            for index, pair in enumerate(self.labelinput.isotopomer_combination["combinations"], start=1):
                ...
                self.logger.debug(f"Folder path : {self.res_folder_path}\n Dataframe {index:02d}:\n {df}")
        ...
        self.logger.debug(f"Elements in the vmtf dictionary {self.vmtf_element_dict}")
        self.logger.info(f"Folder containing linp files has been generated on this directory : {self.res_folder_path}.\n")
    ...
    def generate_vmtf_file(self):
        ...
        self.logger.debug(f"Creation of the vmtf file containing these files :\n {df}")
        df.to_csv(f"{self.res_folder_path}/{self.prefix}.vmtf", sep="\t", index=False)

        self.logger.info("Vmtf file has been generated in the linp folder.")
    ...
    def influx_simulation(self, command_list):
        ...
        self.logger.info("Influx_s is running...")
        ...
        self.logger.info("You can check your results in the current directory")
    ...
    def generate_summary(self):
        ...
        for (root, folders_names, files_names) in os.walk(self.res_folder_path.absolute()):
            if root.endswith("_res"):
                self.tvar_sim_paths += [Path(f"{root}/{files}") for files in files_names if files.endswith('.tvar.sim')]
        self.logger.debug(f"List of tvar.sim file paths : \n {self.tvar_sim_paths}")
        ...
        # self.logger.debug(f"tvar sim dataframes {tvar_sim_dataframes}")
        ...
        # self.logger.debug(f"tvar_sim_dataframes: {tvar_sim_dataframes}")
        ...
        self.logger.debug(f"Merged_tvar : {merged_tvar}")
        ...
        self.logger.debug(f"{self.summary_dataframe}")

        # Creating a Styler object for the summary_dataframe DataFrame
        self.summary_dataframe_styler=self.summary_dataframe.style.apply(
            ...
            axis=1)
        self.summary_dataframe_styler.to_excel(f"{self.res_folder_path}/summary.xlsx", index=None)
 
    
    def data_filter(self, fluxes_names:list=None, kind:str=None, pathways:list=None):
        """
        ...
        if fluxes_names:
            self.summary_dataframe = self.summary_dataframe.loc[self.summary_dataframe["Name"].isin(fluxes_names)]
        if kind:
            self.summary_dataframe = self.summary_dataframe.loc[self.summary_dataframe["Kind"] == kind]
        if pathways:
            # list storing all flows concerned by the selected metabolic pathway(s) 
            all_fluxes = []
            for pathway_name in pathways:
                # key "pathway" in dictionary netan contains another dictionary as value
                # In this dictionary, key : pathway names, value : list with fluxes names involved in this pathway as value
                all_fluxes.extend(self.netan["pathway"][pathway_name])
            self.summary_dataframe = self.summary_dataframe.loc[self.summary_dataframe["Name"].isin(all_fluxes)]
            
        self.logger.info(f"Filtered dataframe :\n{self.summary_dataframe}")
...
if __name__ == "__main__":
    ...
    test = Process()
    test.get_path_input_folder(r"C:\Users\kouakou\Documents\IsoDesign\isodesign\test_data\acetate_LLE")
    ...
    # # test.data_filter(pathway=["GLYCOLYSIS", "BIOMASS"], kind="NET")
    # # # test for dataframe filtering and number of marked shapes 
    
    sd_sum = Score(test.summary_dataframe, "sum_sd")
    fluxes_sd = Score(test.summary_dataframe, "flux_sd", threshold = 100)

    handler = (sd_sum / fluxes_sd)  
    print(handler)
