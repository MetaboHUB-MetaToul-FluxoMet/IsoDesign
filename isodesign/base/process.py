...
class Process:
    ...
    def generate_linp_files(self):
        ...
        with open(os.path.join(str(self.results_dir_path), f'{self.metabolic_netw_model_name}_IDs_combinations.tsv'), 'w', encoding="utf-8") as f:
            ...
            for index, dataframes in self.linp_dataframes.items():
                ...
                self.linp_files_infos[f"{index}"] = linp_files_infos(self.obtain_nb_labeled_inputs(df["Isotopomer"]), df["Price"].sum())
    ...
    def obtain_nb_labeled_inputs(self, df: pd.Series):
        """
        Obtain the number of labeled inputs from a linp file.

        :param df: pandas Series containing the "Isotopomer" column from a linp dataframe
        """
        return len([isotopomer for isotopomer in df if "1" in isotopomer])
